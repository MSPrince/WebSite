...
function Avatar({ userId, name, imgUrl, width, height }) {
    ...
    >
      {imgUrl ? (
        <img
          ...
          height={height}
          alt={name}
          className="overflow-hidden rounded-full h-full w-full"
        />
      ) : name ? (
        <div
          style={{ width: `${width}px`, height: `${height}px` }}
      ...
      {isOnline && (
        <div className="bg-green-600 p-[6px] absolute -top-1 -right-1 z-10 rounded-full "></div>
      )}
    </div>
  );
}
